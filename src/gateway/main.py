...
class GatewayService:
    ...
    def _serve_one_client(self, sock):
        ...
        def forward_to_client(message, ack, _nack):
            try:
            ...
            try:
                ...
                logging.exception("error decoding result message; ignoring for EOF detection")
                return
            if decoded.get("type") == "eof":
                logging.info("Received EOF on result queue for client %s", decoded.get("client"))
                done.set()
                try:
        ...
        try:
            ...
            logging.info("Inbound EOF received; forwarding EOF to downstream queues")
            ...
            logging.info("Pipeline complete; closing client socket")
    ...
    def _close_resources(self):
        if self._server_sock is not None:
            try:
                ...
                pass
        for mw in (self._tx_mw, self._accounts_mw):
            if mw is None:
                continue
            try:
                mw.close()
